...
NGL.specialOps.showSurface = function (id, selection, view, keepPrevious) {
    ...
    // ADDITION BY C.L.: Use existing color scheme if possible.
    if (typeof window["colorScheme-" + id] !== 'undefined') {
        proteins.map(protein => {
            protein.addRepresentation("surface", {
                sele: selection,
                colorScheme:  window["colorScheme-" + id],
                surfaceType: "av"
            });
        });
    } else {
        proteins.map(protein => {
            protein.addRepresentation("surface", {
                sele: selection,
                colorScheme: "uniform", // MODIFICATION BY C.L.: Show a uniform color if not specified.
                colorValue: "#DADAFF",
                colorDomain: [-0.3, 0.3],
                surfaceType: "av"
            });
...
NGL.specialOps.doubleLoader = function (stage, partner, resolve) {
    ...
    if (N_proteins === 0) {
        ...
        let m = myData.proteins.filter((prot) => prot.name === partner || prot.value === partner)[0]; // NEED TO CHANGE ACCORDING TO NEW FORMAT.
        let p;
...
NGL.specialOps.load = function (option, noLoadFun, id) {
    ...
    if (typeof option === "undefined") {
        ...
        myData.proteins[id].push(object); // MODIFIED by C.L.
        index = myData.proteins[id].length - 1; // MODIFIED by C.L.
    } else if ((typeof option === "string") && (option === "next")) { // ADDITION BY C.L.: toggle previous/next
        if (myData.currentIndex === myData.proteins[id].length - 1) { // MODIFIED by C.L.
            index = 0;
        } else {
            index = myData.currentIndex + 1;
        }
    } else if ((typeof option === "string") && (myData.proteins[id].some(v => v.name === option))) { // MODIFIED by C.L.
        index = myData.proteins[id].map(v => v.name).indexOf(option); // MODIFIED by C.L.
    } else if ((typeof option === "string") && (myData.proteins[id].some(v => v.value === option))) { // MODIFIED by C.L.
        index = myData.proteins[id].map(v => v.value).indexOf(option); // MODIFIED by C.L.
    } else if ((typeof option === "string") && (option.length === 4)) { // user gave pdb code that is new.
        myData.proteins[id].push({type: 'rcsb', value: option.slice(0, 4)}); //no chains please. // MODIFIED by C.L.
        index = myData.proteins[id].length - 1; // MODIFIED by C.L.
    } else if ((!isNaN(parseInt(option))) && (parseInt(option) < myData.proteins[id].length)) { //user gave a number as a string. // MODIFIED by C.L.
    ...
    if ((index === myData.currentIndex)) {
        ...
        myData.currentChain = myData.proteins[id][index].chain // MODIFIED by C.L.
    }
    // deal with image.
    if ($('#' + id + ' img')) {
    ...
    // ADDITION BY C.L.: Disallow unnecessary mouse controls.
    NGL.getStage(id).mouseControls.remove("hoverPick");
    NGL.getStage(id).mouseControls.remove("clickPick-left");
    NGL.getStage(id).mouseControls.remove("clickPick-right");
    NGL.getStage(id).mouseControls.remove("clickPick-middle");
    NGL.getStage(id).mouseControls.remove("clickPick-ctrl-left");

    //new model. Force reset
    if (myData.proteins[id][index].type === 'url') { // MODIFIED by C.L.
        return NGL.getStage(id).loadFile(myData.proteins[id][index].value, {'firstModelOnly': true}).then(function (protein) { // MODIFIED by C.L.
            if (noLoadFun === false || noLoadFun === undefined) {
                NGL.specialOps._run_loadFx(protein, myData.proteins[id][index].loadFx); // MODIFIED by C.L.
            }
        });
    } else if (myData.proteins[id][index].type === 'data') { // MODIFIED by C.L.
        var ext = myData.proteins[id][index].ext || 'pdb'; // MODIFIED by C.L.
        if (!!myData.proteins[id][index].isVariable) { // MODIFIED by C.L.
            let varname = myData.proteins[id][index].value.replace(/\W/g, ''); // MODIFIED by C.L.
            if (window[varname] !== undefined) {
                return NGL.getStage(id).loadFile(new Blob([window[varname], {type: 'text/plain'}]), {
                    ...
                    .then(function (protein) {
                        if (noLoadFun === false || noLoadFun === undefined) {
                            NGL.specialOps._run_loadFx(protein, myData.proteins[id][index].loadFx);
                        }
                    });
            } else { //async issue.
                setTimeout(() => NGL.specialOps.load(option, noLoadFun, id), 300);
            }
        } else if (typeof myData.proteins[id][index].value === 'string') {
            return NGL.getStage(id).loadFile(new Blob([myData.proteins[id][index].value, {type: 'text/plain'}]), { // MODIFIED by C.L.
                ext: ext,
                firstModelOnly: true
            }).then(function (protein) {
                if (noLoadFun === false || noLoadFun === undefined) {
            ...
            return NGL.getStage(id).loadFile(myData.proteins[id][index].value, { // MODIFIED by C.L.
                ext: ext,
                firstModelOnly: true
            }).then(function (protein) {
                if (noLoadFun === false || noLoadFun === undefined) {
    ...
    } else if (myData.proteins[id][index].type === 'none') { // MODIFIED by C.L.
    ...
    } else if (myData.proteins[id][index].value.replace('rcsb://', '').trim().length === 4) { //PDB code. // MODIFIED by C.L.
        return NGL.getStage(id).loadFile('rcsb://' + myData.proteins[id][index].value.replace('rcsb://', '').toLowerCase().slice(0, 4), {firstModelOnly: true}).then(function (protein) { // MODIFIED by C.L.
            if (noLoadFun === false || noLoadFun === undefined) {
                NGL.specialOps._run_loadFx(protein, myData.proteins[id][index].loadFx); // MODIFIED by C.L.
            }
        });
...
NGL.specialOps.multiLoader = function (id, proteins, backgroundColor, startIndex) {
    ...
    // check for awkward case it has already been started.
    if (typeof window.myData === 'object') {
        if (!(id in window.myData.proteins)) {
            window.myData.proteins[id] = proteins; // MODIFIED by C.L.
        } else window.myData.proteins[id].push(...proteins); // MODIFIED by C.L.
        ...
        let protein_dict = {};
        protein_dict[id] = proteins;
        window.myData = {
            currentIndex: -1,
            currentIndices: {},
            proteins: protein_dict,
            id: id,
            ids: [id],
            backgroundColor: 'white',
            backgroundColors: {}
...
NGL.specialOps.prolink = function (prolink) { //prolink is a JQuery object.
    ...
    function move() {
        ...
        if (view === 'auto') { //special view case.
            ...
            // ADDITION BY C.L.: Reload current structure.
            let proteins = NGL.getStage(id).getComponentsByType('structure');
            proteins.map(protein => protein.removeAllRepresentations());

            // FLAG REFACTOR
            NGL.specialOps._run_loadFx(stage.getComponentByType('structure'), myData.proteins[id][myData.currentIndices[id]].loadFx); // Modified by C.L.
...
$.prototype.viewport = function () {
    ...
    // DELETION BY C.L.: Do not set bare minima for width and height.
    // sort attributes
    if (!$(this).attr('id')) {
...
// DELETION BY C.L.: Do not initialize viewports automatically.